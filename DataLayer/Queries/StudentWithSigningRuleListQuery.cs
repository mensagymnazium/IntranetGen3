	...
	protected override IQueryable<StudentWithSigningRuleListItemDto> Query()
	{
		var data = studentDataSource.Data
			...
			.Select(x => new StudentWithSigningRuleListItemDto
			{
				...
				MainRegistrationsCount = x.signingRule.Registrations.Count(r => (r.RegistrationType == StudentRegistrationType.Main) && (r.StudentId == x.student.Id)),
				SecondaryRegistrationsCount = x.signingRule.Registrations.Count(r => (r.RegistrationType == StudentRegistrationType.Secondary) && (r.StudentId == x.student.Id)),
			});
